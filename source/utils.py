...
#cfg model optimizer lr_scheduler logger
# -----------------------------------------------------------------------------
...
def save_best_checkpoint_i(cfg, epoch, model, best_auc, best_epoch, optimizer, lr_scheduler, logger,i):
    ...
    save_path = os.path.join(ckpt_dir, f'best_ckpt_{i}.pth')
...
# def load_best_results(cfg,logger):
#     loaded_best_models = []
#
#     str1 = cfg.OUTPUT_DIR+"/checkpoints/"
#     for i in range(5):
#         str = str1+"best_ckpt_"+i+".ckpt"
...
def load_best_result_i(cfg, model, logger,i):
    ckpt_dir = os.path.join(cfg.OUTPUT_DIR, "checkpoints")
    best_ckpt_path = os.path.join(ckpt_dir, f'best_ckpt_{i}.pth')
    #logger.info(f'Ckpt loading: {best_ckpt_path}')
    ckpt = torch.load(best_ckpt_path)
    model.load_state_dict(ckpt['model'])
    best_epoch = ckpt['best_epoch']
