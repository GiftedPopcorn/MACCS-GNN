...
class DotProductAttention(nn.Module):
    ...
    def forward(self, input1, input2):
        energy = torch.matmul(input1, input2.transpose(0, 1))
        attention_weights = torch.softmax(energy, dim=1)
        output = torch.matmul(attention_weights, input1)
        return output
...
class NTNConv(MessagePassing):
    ...
    def message(self, x_i, x_j, edge_attr):
        #source_to_target  x_i is target
...
class HiGNN(torch.nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, edge_dim, num_layers,
                 slices, dropout, f_att=True, dot_att = True ,r=4, brics=True, cl=False):
        ...
        if self.dot_att:
            self.dotproduct_att = DotProductAttention()
        ...
        #if self.brics and self.dot_att:
        #     self.out = Linear(3 * hidden_channels, out_channels)
        #elif self.dot_att or self.brics:
    ...
    def forward(self, data):
        ...
        if self.dot_att:
            mol_vec = self.dotproduct_att(mol_vec,MACCS)
        mol_vec = F.relu(mol_vec)
        if self.brics:
            # get fragment input
            fra_x = data.x
