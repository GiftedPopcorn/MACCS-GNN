...
from utils import save_best_checkpoint_i, load_best_result_i
...
def validate(cfg, model, criterion, dataloader, epoch, device, logger, eval_mode=False):
    ...
    if eval_mode:
        ...
        return val_results,y_pred_list,y_label_list
...
def train(cfg, logger):
    ...
    for epoch in range(cfg.TRAIN.START_EPOCH, cfg.TRAIN.MAX_EPOCHS):
        ...
        # 2: Upadate learning rate  学习率是reduce
        if not isinstance(lr_scheduler, NoamLR):
            lr_scheduler.step(val_loss)

        # 3: Print results show_freq = 5
        ...
        if epoch < 5:
            ...
            worst_score = min(best_set_scores)
            worst_model_idx = best_set_scores.index(min(best_set_scores))
            if cfg.DATA.TASK_TYPE == 'classification' and val_score > worst_score or \
                    cfg.DATA.TASK_TYPE == 'regression' and val_score < worst_score:
                best_score, best_epoch = val_score, epoch
                count = worst_model_idx
                save_best_checkpoint_i(cfg, epoch, model, best_score, best_epoch, optimizer, lr_scheduler, logger,
                                       count)
                best_set_scores[worst_model_idx] = best_score
                early_stop_cnt = 0
            else:
                early_stop_cnt += 1
        # 6: Early stopping.
    ...
    score = integrated_validation(cfg, model, criterion, test_loader, logger,device)
    logger.info(f'Seed {cfg.SEED} ==> test {cfg.DATA.METRIC} = {np.nanmean(score):.3f}')
    return score


def integrated_validation(cfg, model, criterion, test_loader, logger,device):
    path_str1 = os.path.join(cfg.OUTPUT_DIR, "checkpoints")
    dicts = []
    for i in range(5):
        path_str = os.path.join(path_str1, f"best_ckpt_{i}.pth")
        if os.path.isfile(path_str):
            model, best_epoch = load_best_result_i(cfg, model, logger, i)
            score , pred , label= validate(cfg, model, criterion, test_loader, best_epoch, device, logger=logger, eval_mode=True)
            dicts.append(pred)
    # 初始化一个字典来存储累加的值
    sum_dict = {key: [] for key in range(len(cfg.DATA.TASK_NAME))}
    # 遍历所有字典，并将值累加到sum_dict中
    count = 0
    for d in dicts:
        if count == 0 :
            for i, task in enumerate(cfg.DATA.TASK_NAME):
            ...
            for i, task in enumerate(cfg.DATA.TASK_NAME):
                sum_dict[i] = [a + b for a, b in zip(sum_dict[i], d[i])]
    # 计算平均值并存储在新的字典中
    avg_dict = {key: [] for key in range(len(cfg.DATA.TASK_NAME))}
    for key in range(len(cfg.DATA.TASK_NAME)):
        value = sum_dict[key]
        for i in range(len(value)):
            value[i] = value[i]/len(dicts)
        avg_dict[key] = value
    val_results = []
    metric_func = get_metric_func(metric=cfg.DATA.METRIC)
    for i, task in enumerate(cfg.DATA.TASK_NAME):
        if len(label[i]) == 0:
            continue
        val_results.append(metric_func(label[i], avg_dict[i]))
    return val_results
