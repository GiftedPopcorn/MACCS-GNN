...
def build_dataset(cfg, logger):
    ...
    #task_name = get_task_names("../data/bbbp/raw/bbbp.csv")
...
def shuffle_dataset_random(train_dataloader, valid_dataloader, test_dataloader,cfg,logger):
    
    pyg_dataset = torch.utils.data.ConcatDataset([train_dataloader.dataset, valid_dataloader.dataset,test_dataloader.dataset])
    ...
    trn = torch.utils.data.Subset(pyg_dataset,trn_id)
    val = torch.utils.data.Subset(pyg_dataset, val_id)
    test = torch.utils.data.Subset(pyg_dataset, test_id)

    train_dataloader = DataLoader(trn, batch_size=cfg.DATA.BATCH_SIZE, shuffle=True)
    valid_dataloader = DataLoader(val, batch_size=cfg.DATA.BATCH_SIZE)
    test_dataloader = DataLoader(test, batch_size=cfg.DATA.BATCH_SIZE)
