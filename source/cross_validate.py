...
def cross_validate(cfg, logger):
    ...
    if cfg.NUM_FOLDS == 10:
        ...
        cfg.NUM_FOLDS = 10
        cfg.freeze()
    for fold_num in range(num,cfg.NUM_FOLDS):
        cfg.defrost()
...
def hyperopt(cfg, logger):
    ...
    def objective(hyperparams):
        ...
        if hyper_resume_cfg.TRAIN.RESUME is not None:
            folder_name = f'round_{hyper_resume_cfg.HYPER_COUNT}'
            hyper_resume_cfg.defrost()
            hyper_resume_cfg.OUTPUT_DIR = os.path.join(hyper_resume_cfg.OUTPUT_DIR, folder_name)
            hyper_resume_cfg.freeze()
        ...
        if hyper_resume_cfg.TRAIN.RESUME is not None:
            hyperparams["MODEL.HID"] = hyper_resume_cfg.MODEL.HID
            hyperparams["MODEL.SLICES"] = hyper_resume_cfg.MODEL.SLICES
            hyperparams["MODEL.DROPOUT"] =hyper_resume_cfg.MODEL.DROPOUT
            hyperparams["MODEL.DEPTH"] =hyper_resume_cfg.MODEL.DEPTH
            hyperparams["TRAIN.OPTIMIZER.BASE_LR"] = hyper_resume_cfg.TRAIN.OPTIMIZER.BASE_LR          
            hyperparams["TRAIN.OPTIMIZER.WEIGHT_DECAY"] = hyper_resume_cfg.TRAIN.OPTIMIZER.WEIGHT_DECAY
        logger.info(hyperparams)

        hyper_sum_cfg = None
        # Cross validate
        if hyper_resume_cfg.TRAIN.RESUME is not None:
            mean_score, std_score = cross_validate(hyper_resume_cfg, logger)
            hyper_resume_cfg.defrost()
            hyper_resume_cfg.TRAIN.RESUME = None
            hyper_resume_cfg.freeze()
            hyper_sum_cfg =deepcopy(hyper_resume_cfg)
        else:
            mean_score, std_score = cross_validate(hyper_cfg, logger)
            hyper_sum_cfg = deepcopy(hyper_cfg)
        cfg.defrost()
        cfg.TRAIN.RESUME = None
        cfg.NUM_FOLDS = 10
        cfg.freeze()
        # Record results
        temp_model = build_model(hyper_sum_cfg)
